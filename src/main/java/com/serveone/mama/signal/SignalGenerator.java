...
public class SignalGenerator {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            응답은 반드시 JSON 객체 하나로만 출력합니다. 마크다운, 설명, 코드펜스를 추가하지 마세요.
            JSON 스키마:
            {
              "ticker": "<6자리 종목코드>",
    ...
    public SignalGenerator(ClaudeClient claude, ObjectMapper mapper) {
        this.claude = claude;
        this.mapper = mapper;
    }
    ...
    public Signal generate(DisclosureItem item) {
        ...
        String text = claude.complete(SYSTEM_PROMPT, buildUserPrompt(item), MAX_TOKENS);
